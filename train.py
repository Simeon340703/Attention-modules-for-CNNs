...
from torch.optim.lr_scheduler import StepLR, ExponentialLR,CosineAnnealingLR, MultiStepLR
...
parser.add_argument('--att-type', type=str, choices=['BAM', 'CBAM','ECAAttention', 'SEAttention','ShuffleAttention'], default=None)
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.distributed:
        if args.dist_url == 'env"//' and args.rank == -1:
            # port = find_free_port()
            # args.dist_url = f"tcp://127.0.01:{port}"
  ...
  #  model = ResNet50()
    ...
    criterion = CrossEntropyLabelSmooth().cuda(args.gpu)# nn.CrossEntropyLoss().cuda(args.gpu)
    ...
    scheduler = MultiStepLR(optimizer, milestones=[30, 60, 90, 120], gamma=0.1, verbose=True)
